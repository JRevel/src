...
Ball::Ball(Player &player, Vec pos, double r, double m, Vec speed, int maxHealth) : MovingEntity(player, pos, speed, BALL, maxHealth), m_m(m), m_r(r), m_movement(NULL)
{
    ...
    m_sprite.setScale(sf::Vector2f(2*r/BALL_REAL_R, 2*r/BALL_REAL_R));
    m_sprite.setColor(player.getBaseColor());
    m_hoveredSprite.setTexture(*texture);
    m_hoveredSprite.setPosition(x-r, y-r);
    m_hoveredSprite.setScale(sf::Vector2f(2*r/BALL_REAL_R, 2*r/BALL_REAL_R));
    ...
    m_selectedSprite.setScale(sf::Vector2f(2*r/selectedTexture->getSize().x, 2*r/selectedTexture->getSize().y));
    m_selectedSprite.setColor(player.getSelectedColor());
    m_gui = new GuiBall(this);
}
...
{
    sf::Texture *texture = ImageManager::instance.getTexture("ball");
    m_sprite.setPosition(x-texture->getSize().x/2*m_sprite.getScale().x, y-texture->getSize().y/2*m_sprite.getScale().y);
...
{
    sf::Texture *texture = ImageManager::instance.getTexture("selectedBall");
    m_selectedSprite.setPosition(x-texture->getSize().x/2*m_sprite.getScale().x, y-texture->getSize().y/2*m_sprite.getScale().y);
    render.draw(m_selectedSprite);
}

void Ball::drawHovered(sf::RenderTarget &render)
{

    sf::Texture *texture = ImageManager::instance.getTexture("ball");
    m_hoveredSprite.setPosition(x-texture->getSize().x/2*m_sprite.getScale().x, y-texture->getSize().y/2*m_sprite.getScale().y);
    m_hoveredSprite.setColor(sf::Color(255, 255, 0, 100+abs(getLivingTime()%100-50)));
...
void Ball::drawForeground(sf::RenderTarget &render, Vec mousePos)
{
    if(/*distSquared(getPos(), mousePos) < 10000 ||*/ getLastHitTime() < 150)
        drawHealthBar(render, getPos()-Vec(m_r*0.8, m_r/4), Vec(m_r*2*0.8, m_r/2), std::max(1-distSquared(getPos(), mousePos)/10000, getLastHitTime() < 150 ? 1-getLastHitTime()/150.0:0));
}
