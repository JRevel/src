...
MovementHandler::MovementHandler(std::vector<Ball*> balls, Vec target) : m_balls(balls), m_target(target), m_isOver(false), m_endTime(0), m_ctrl(false)
{
    for(int i=0; i<m_balls.size(); i++)
        m_balls[i]->setMovement(this);
}

MovementHandler::MovementHandler(Ball* ball, Vec target) : m_target(target), m_isOver(false), m_endTime(0), m_ctrl(false)
{
