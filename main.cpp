...
int main()
{
    ...
    Vec blueCenter(100, 500), redCenter(500, 100);

    JoypadPlayerController joypadController = JoypadPlayerController(event, 0, world.getDim(), blueCenter);
    int *keys = new int[8];
    ...
    KeyboardPlayerController keyController = KeyboardPlayerController(event, keys, world.getDim(), redCenter);
...
//    bool zPressed = false, select = false;
    ...
    world.addEntity(new MainBuilding(*bluePlayer, blueCenter));
    world.addEntity(new MainBuilding(*redPlayer, redCenter));
