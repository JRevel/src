...
std::vector<Entity*>& CollisionHandler::getClip(int x, int y) const
{
    return m_clips[std::min(x, m_clipsX-1)+m_clipsX*std::min(y, m_clipsY-1)];
}

std::vector<Entity*> CollisionHandler::getEntitiesAt(Vec A) const
{
    double clipDimX = m_world.getDim().x/m_clipsX;
    double clipDimY = m_world.getDim().y/m_clipsY;
    ...
    for(int x = std::max(0, Ax-1); x<std::min(m_clipsX, Ax+2); x++)
    {
        for(int y = std::max(0, Ay-1); y<std::min(m_clipsY, Ay+2); y++)
        {
            std::vector<Entity*> clip = getClip(x, y);
            for(int i=0; i<clip.size(); i++)
            {
                if(clip[i]->isInside(A))
...
std::vector<Entity*> CollisionHandler::getEntitiesInRect(Vec A, Vec B) const
{
    double clipDimX = m_world.getDim().x/m_clipsX;
    double clipDimY = m_world.getDim().y/m_clipsY;
    int Ax = (int)(A.x/clipDimX);
...
std::vector<Entity*> CollisionHandler::selectEntitiesInRect(const Player *player, Vec A, Vec B) const
{
