...
public:
    ...
    std::vector<Entity*>& getClip(int x, int y) const;
    std::vector<Entity*> selectEntitiesInRect(const Player *Player, Vec A, Vec B) const;
    std::vector<Entity*> getEntitiesInRect(Vec A, Vec B) const;
    std::vector<Entity*> getEntitiesAt(Vec A) const;
