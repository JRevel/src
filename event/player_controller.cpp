...
{
    double f = 0.4+((100-m_event.joystickPosition(m_id, 2))/200.0)*0.6;
    Displayer::instance.addValue(Vec(50, 50), "test", draw::to_string(m_event.joystickPosition(m_id, 2))+", " +draw::to_string(m_event.joystickPosition(m_id, 3)));
    if(abs(m_event.joystickPosition(m_id, 0))>5)
        m_mousePos.x += m_event.joystickPosition(m_id, 0)*m_speed*f;
    if(abs(m_event.joystickPosition(m_id, 1))>5)
        m_mousePos.y += m_event.joystickPosition(m_id, 1)*m_speed*f;
    PlayerController::update();
}
