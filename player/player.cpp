...
Player::Player(PlayerController &controller, World &world, sf::Color baseCol, sf::Color selectedCol, sf::Color chargingCol, sf::Color selectedChargingCol) :
        ...
        m_gui(new GuiIngame(&m_selection, m_hovered))
{
    m_zooms[0] = 1.3;
    m_zooms[1] = 1;
    m_zooms[2] = 0.7;
...
void Player::update()
{
    ...
    if(m_controller.pressed(SELECT) && !m_gui->mouseOnGui(m_controller.mousePos(), Vec(0, 0), m_drawDim))
    {
    ...
    if(!m_gui->mouseOnGui(m_controller.mousePos(), Vec(0, 0), m_drawDim) && m_selection.size() > 0)
        sendOrder();
    if(!m_gui->mouseOnGui(m_controller.mousePos(), Vec(0, 0), m_drawDim))
    {
        std::vector<Entity*> hovered = m_world.getEntitiesAt(screenToWorld(m_controller.mousePos()));
        if(hovered.size() > 0)
            m_hovered = hovered[0];
    }
...
void Player::draw(sf::RenderWindow &render) const
{
    ...
    for(int i=0; i<toDraw.size(); i++)
    {
        ...
        if(toDraw[i] == m_hovered)
        {
            toDraw[i]->drawHovered(render);
        }
    }
    for(int i=0; i<m_selection.size(); i++)
    {
